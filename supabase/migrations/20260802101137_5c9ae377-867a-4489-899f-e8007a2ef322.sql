...
grant select on public.platform_provider_connections to authenticated;
grant all on public.platform_provider_connections to service_role;

grant select on public.platform_provider_resources to authenticated;
grant all on public.platform_provider_resources to service_role;

grant select, insert on public.platform_provider_commands to authenticated;
grant all on public.platform_provider_commands to service_role;

grant select on public.platform_provider_events to authenticated;
grant all on public.platform_provider_events to service_role;

grant select, insert on public.platform_onboarding_cases to authenticated;
grant all on public.platform_onboarding_cases to service_role;
...
create policy platform_provider_resources_authenticated_read
on public.platform_provider_resources for select
to authenticated
using (true);

create policy platform_provider_events_authenticated_read
on public.platform_provider_events for select
to authenticated
using (true);

create policy platform_provider_commands_own_read
on public.platform_provider_commands for select
to authenticated
...
create trigger platform_provider_connections_updated_at
before update on public.platform_provider_connections
for each row execute function public.update_updated_at_column();

create trigger platform_provider_resources_updated_at
before update on public.platform_provider_resources
for each row execute function public.update_updated_at_column();

create trigger platform_onboarding_cases_updated_at
before update on public.platform_onboarding_cases
for each row execute function public.update_updated_at_column();
