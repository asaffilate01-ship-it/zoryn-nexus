  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-6 px-5 py-10">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          label="Dead-letter commands"
          value={String(deadCommands)}
          help="Need manual replay"
        />
        <MetricCard
          label="Dead-letter events"
          value={String(deadEvents)}
          help="Unmapped or failing"
        />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Panel title="Provider commands" subtitle="Outbound Swan, Adyen and Rewards work items">
          ...
          {data.commands.map((c) => (
            <Row
              key={c.id}
              primary={`${c.provider} · ${c.command_type}`}
              secondary={`Attempt ${c.attempt_count}${c.last_error ? ` · ${c.last_error}` : ""}`}
              status={c.status}
            />
          ))}
        </Panel>
        ...
        <Panel
          ...
          subtitle="Inbound provider notifications and their processing state"
        >
          {data.events.length === 0 && <Empty label="No events received." />}
          {data.events.map((e) => (
            <Row
              key={e.id}
              primary={`${e.provider} · ${e.event_type}`}
              secondary={`Attempt ${e.attempt_count}${e.last_error ? ` · ${e.last_error}` : ""}`}
        ...
        <Panel title="Incidents" subtitle="Declared service incidents and their lifecycle">
          ...
          {data.incidents.map((i) => (
            <Row
              key={i.id}
              primary={`${i.reference} · ${i.title}`}
              secondary={`Severity ${i.severity}`}
              status={i.status}
            />
          ))}
        </Panel>
        ...
        <Panel
          ...
          subtitle="Queued customer messaging, delivered out of band"
        >
          {data.outbox.length === 0 && <Empty label="Outbox empty." />}
          {data.outbox.map((n) => (
            <Row
              key={n.id}
              primary={n.template_key}
              secondary={`${n.channel} · attempt ${n.attempt_count}`}
              status={n.status}
            />
          ))}
        </Panel>
        ...
        <Panel title="Support cases" subtitle="Customer and merchant cases with SLA priority">
          ...
          {data.supportCases.map((s) => (
            <Row
              key={s.id}
              primary={`${s.reference} · ${s.subject}`}
              secondary={`${s.case_type} · ${s.priority}`}
              status={s.status}
            />
          ))}
        </Panel>
        ...
        <Panel title="Reconciliation" subtitle="Provider ledger versus Zoryn ledger runs">
          ...
          {data.reconciliation.map((r) => (
            <Row
              key={r.id}
              primary={`${r.provider} · ${r.run_type}`}
              secondary={`${r.matched_count} matched · ${r.mismatched_count} mismatched`}
              status={r.status}
            />
          ))}
        </Panel>
