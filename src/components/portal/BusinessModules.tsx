    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={money(b.pendingSettlement)}
          hint={t("Settles next batch")}
        />
        <StatCard
          label={t("Active staff cards")}
          value={String(b.team.filter((t) => !t.frozen).length)}
          hint={t("{count} team members", { count: b.team.length })}
        />
      </div>
        ...
        <Panel title={t("Cash flow")} subtitle={t("Money in vs money out this period")}>
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-1">
                <Progress value={(inflow / max) * 100} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>{t("Money out")}</span>
                <span>{money(outflow)}</span>
              </div>
              <div className="mt-1">
                <Progress value={(outflow / max) * 100} />
              </div>
            </div>
          ...
          <div className="divide-y divide-border">
            {b.txns.slice(0, 6).map((t2) => (
              <div key={t2.id} className="flex items-center justify-between gap-4 py-3">
                ...
                <span
                  className={
                    t2.amount > 0 ? "font-display text-sm text-primary" : "font-display text-sm"
                  }
                >
                  {t2.amount > 0 ? "+" : ""}
                  {money(t2.amount)}
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel
          title={t("Supplier SEPA payment")}
          subtitle={t("Debited from {iban}", { iban: b.iban })}
        >
          <div className="space-y-3">
            <Field label={t("Supplier")}>
              ...
              >
                {b.suppliers.map((s) => (
                  <option key={s.iban} value={s.name}>
                    {s.name}
                  </option>
                ))}
              </select>
            ...
            <Button
              onClick={() =>
                setError(
                  supplierPayment({ name: supplier, iban, amount: Number(amount), reference }),
                )
              }
            >
        ...
        <Panel
          title={t("Create payment link")}
          subtitle={t("Share a hosted checkout link with a customer")}
        >
          <div className="space-y-3">
            ...
            <div className="divide-y divide-border">
              {b.links.length === 0 ? (
                <Empty>{t("No payment links yet.")}</Empty>
              ) : (
                b.links.map((l) => (
                  <div key={l.id} className="flex items-center justify-between gap-3 py-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="mt-1">
                        <Badge tone={l.status === "paid" ? "good" : "neutral"}>{t(l.status)}</Badge>
                      </div>
                    </div>
                  </div>
                ))
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        />
        <StatCard
          label={t("Combined limits")}
          value={money(b.team.reduce((a, t) => a + t.limit, 0))}
        />
      </div>
      <Panel
        title={t("Team members & staff cards")}
        subtitle={t("Roles, per-card limits and utilisation")}
      >
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                ...
                <th className="pb-3">{t("Card")}</th>
                <th className="pb-3">{t("Utilisation")}</th>
                <th className="pb-3">{t("Limit (€)")}</th>
            ...
            <tbody className="divide-y divide-border">
              {b.team.map((t2) => (
                <tr key={t2.id}>
                  ...
                  <td className="py-3">
                    <div className="w-32">
                      <Progress value={(t2.spent / t2.limit) * 100} />
                    </div>
                    <small className="text-xs text-muted-foreground">
                      {t("{spent} of {limit}", { spent: money(t2.spent), limit: money(t2.limit) })}
                    </small>
                  </td>
                  <td className="py-3">
                    <input
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <Badge tone={t2.frozen ? "bad" : "good"}>
                        {t2.frozen ? t("Frozen") : t("Active")}
                      </Badge>
                      <Button
                        variant="ghost"
                        className="px-2 py-1 text-xs"
                        onClick={() => toggleTeamCard(t2.id)}
                      >
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          value={money(Math.floor(b.points / 500) * 5)}
          hint={t("500 points = €5")}
        />
      </div>
      <Panel
        title={t("Convert points to cash")}
        subtitle={t("500 points converts into €5, credited to your chosen cashback destination")}
      >
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button onClick={() => setError(redeemBusinessPoints(Number(points)))}>
            {t("Convert for {amount}", {
              amount: money((Math.floor(Number(points) / 500) || 0) * 5),
            })}
          </Button>
        </div>
        <ErrorText>{error}</ErrorText>
      ...
      <Panel
        title={t("Rewards campaigns")}
        subtitle={t("Team spend automatically earns Zoryn Points")}
      >
          ...
          {[
            ...
            <div key={n} className="rounded-xl border border-border bg-background/40 p-4">
              ...
              <Button
                ...
                onClick={() =>
                  notify(t("{name} activated for the demo organisation.", { name: n ?? "" }))
                }
              >
                {t("Activate")}
              </Button>
