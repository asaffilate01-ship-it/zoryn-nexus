...
function TapToPay() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_1.2fr]">
      <Panel title={t("Tap to Pay")} subtitle={t("Enter any amount and simulate a contactless tap")}>
        {stage === "amount" && (
        ...
        {stage !== "amount" && (
          <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/15 to-accent/10 p-8 text-center">
            <span className="text-[11px] uppercase tracking-widest text-primary">ZorynPay {t("terminal")}</span>
            <p className="mt-3 font-display text-4xl">{money(value)}</p>
            {stage === "waiting" && (
              <>
                <p className="mt-4 text-sm text-muted-foreground">{t("Present card or phone to the reader…")}</p>
                <Button className="mt-5" onClick={() => setStage("reading")}>{t("Simulate customer tap")}</Button>
                <div className="mt-3">
                  <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setStage("amount")}>{t("Cancel")}</Button>
                </div>
              </>
            )}
            {stage === "reading" && <p className="mt-4 animate-pulse text-sm text-muted-foreground">{t("Reading card · authorising…")}</p>}
            {stage === "approved" && (
              <>
                <p className="mt-4 font-display text-2xl text-primary">{t("Approved")}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t("Receipt sent · balance and settlement updated")}</p>
                <Button variant="ghost" className="mt-4 px-3 py-1.5 text-xs" onClick={() => setStage("amount")}>{t("New payment")}</Button>
              </>
            )}
          </div>
        )}
      ...
      <Panel
        ...
        subtitle={t("Merchant balance {balance} · pending settlement {pending}", { balance: money(m.balance), pending: money(m.pendingSettlement) })}
        action={<Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => notify(t("Receipts re-sent to customers (demo)."))}>{t("Resend receipts")}</Button>}
      >
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label={t("Sales")} value={money(total)} hint={t("{count} captured payments", { count: captured.length })} />
        ...
        <StatCard label={t("Average ticket")} value={money(captured.length ? total / captured.length : 0)} />
      </div>
      <TapToPay />
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label={t("Merchant balance")} value={money(m.balance)} />
        <StatCard label={t("Pending settlement")} value={money(m.pendingSettlement)} hint={t("1.5% processing fee")} />
        <StatCard label={t("Settlements paid")} value={String(m.settlements.filter((s) => s.status === "paid").length)} />
      </div>
      <Panel
        title={t("Settlement history")}
        subtitle={t("Instant settlement pays out to the business account")}
        action={<Button onClick={settleNow}>{t("Settle {amount} now", { amount: money(m.pendingSettlement) })}</Button>}
      >
        <div className="overflow-x-auto">
          <table className="w-full min-w-[520px] text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                <th className="pb-3">{t("Date")}</th><th className="pb-3">{t("Gross")}</th><th className="pb-3">{t("Fees")}</th><th className="pb-3">{t("Net")}</th><th className="pb-3">{t("Status")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {m.settlements.map((s) => (
                <tr key={s.id}>
                  ...
                  <td className="py-3"><Badge tone={s.status === "paid" ? "good" : "warn"}>{t(s.status.replace("_", " "))}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="divide-y divide-border">
          {m.payments.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-3 py-3">
              ...
              {p.status === "captured" ? (
                <Button variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => refundPayment(p.id)}>{t("Refund")}</Button>
              ) : (
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Panel title={t("Create payment link")} subtitle={t("Share by QR code, email or chat")}>
        <div className="space-y-3">
          <Field label={t("Description")}><input className={inputClass} value={desc} maxLength={180} onChange={(e) => setDesc(e.target.value)} /></Field>
      ...
      <Panel title={t("Active links")}>
        {m.links.length === 0 ? <Empty>{t("No links yet.")}</Empty> : (
          <div className="divide-y divide-border">
            {m.links.map((l) => (
              <div key={l.id} className="flex items-center justify-between gap-3 py-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" className="px-2 py-1 text-xs" onClick={() => notify(t("{ref} copied (demo).", { ref: l.reference }))}>{t("Copy")}</Button>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      {state.merchant.terminals.map((term) => (
        <Panel key={term.id} title={term.name} subtitle={t("{location} · firmware {firmware}", { location: term.location, firmware: term.firmware })}>
          <div className="flex items-center justify-between">
            <Badge tone={term.status === "online" ? "good" : term.status === "charging" ? "warn" : "bad"}>{t(term.status)}</Badge>
            <span className="text-sm text-muted-foreground">{term.battery}%</span>
          </div>
          <div className="mt-3"><Progress value={term.battery} /></div>
          <Button variant="ghost" className="mt-4 px-3 py-1.5 text-xs" onClick={() => notify(t("{name} diagnostics sent (demo).", { name: term.name }))}>
            {t("Run diagnostics")}
          </Button>
        </Panel>
      ))}
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard label={t("Stamps issued")} value={l.stamps.toLocaleString("de-DE")} hint={t("Increases with every Tap to Pay")} />
          ...
          {l.campaigns.map((c) => (
            <div key={c.name} className="rounded-xl border border-border bg-background/40 p-4">
              <div className="flex items-center justify-between">
                ...
                <Badge tone={c.active ? "good" : "neutral"}>{c.active ? t("Active") : t("Paused")}</Badge>
              </div>
              <p className="mt-1 text-xs text-muted-foreground">{t("{reward} · {joined} members joined", { reward: c.reward, joined: c.joined })}</p>
              <Button variant="ghost" className="mt-3 px-3 py-1.5 text-xs" onClick={() => notify(t("{name} updated (demo).", { name: c.name }))}>{t("Edit campaign")}</Button>
            </div>
          ))}
        </div>
      </Panel>
